...
价值投资视角的红利决策助手：输入任意 A 股代码回测「买入持有 + 分红再投资」，全市场红利股扫描，单股诊断（股息率分位/估值/风险），自选监控。移动端优先。

> **v1.7.1（2026-08-16）**：新增每月追加投入（每月首个交易日、攒一手逻辑、XIRR 含追加现金流）+ 修复决策台秒开（腾讯批量行情）+ 修复 ETF 代码识别 + 修复跨域 Script error 红条
>
> **v1.7.0（2026-08-16）**：送转口径修复（纯转增持股翻倍 bug，旧结果需重算）+ 四 tab 导航（决策台/诊断/对比/回测）+ 全市场红利股扫描器 + 股息率历史分位图 + 自选监控（快照对比/除息日历）+ K线本地缓存（IndexedDB）+ 事件标注（除息/送转标在曲线上）。对比页在第二批开发中。

## 文件说明
- `index.html` — 工具本体（回测核心+样式）
- `data-layer.js` — 数据层（接口/缓存/限流/去重/watchlist）
- `views.js` — 视图层（导航/决策台/诊断/扫描器）
...
- `test-regression.js` — v1.7.0 回归测试（送转口径/零股/边界案例）
...
- **安全边界（2026-08-18）**：密码锁为浏览器端 SHA-256 校验，仅防路人/误触，**不防懂技术者**（GitHub Pages 静态部署无法真正鉴权）；自选/持仓数据存本机 localStorage，不上传任何服务器；需要真正隐私时请用本地部署（`python3 -m http.server`）
