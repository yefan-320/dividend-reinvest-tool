...
async function main() {
  ...
  await S('D2 诊断页内容完整性', async () => {
    ...
    const rhythm = await evalIn(cdp, `document.querySelectorAll('.rhythm-heat, .rhythm-row').length`);
    if (rhythm < 1) {
      ...
      const rhythm2 = await evalIn(cdp, `document.querySelectorAll('.rhythm-heat, .rhythm-row').length`);
