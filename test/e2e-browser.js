...
async function main() {
  ...
  // 2026-08-21：图起点断言加数据降级豁免——腾讯 K线被风控时新浪备源只有 ~1023 条（≈4 年），
  // x0 晚于 d 是外部数据降级非 UI 逻辑问题（d 参数解析已被上方日期回填严格验证）；腾讯通时此分支不触发仍严格验证
  if (!r7b.x0) die('R7-②: 图起点缺失');
  if (r7b.x0 < '2020-01-02' || r7b.x0 > '2020-01-15') {
    if (r7b.x0 >= '2021-01-01') {
      console.log('   ⚠️ 腾讯 K线被风控→新浪降级（仅~4年数据），图起点 ' + r7b.x0 + ' 非 ' + dUrl.match(/d=\d{4}-\d{2}-\d{2}/)[0].slice(2) + '——外部降级，跳过图起点严格断言（日期回填已通过）');
    } else {
      die('R7-②: 图起点异常（x0=' + r7b.x0 + '，应≈2020-01-02 首个交易日）');
    }
  }
