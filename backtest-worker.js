...
self.onmessage = e => {
  ...
  try {
    ...
    const perStock = rows.filter(r => !r._cash).map(r => ({ code: r.code, name: r.name, amount: r.amount, monthly: +r.monthly.toFixed(2), finalValue: r.sim.daily[r.sim.daily.length - 1].value, invested: r.sim.daily[r.sim.daily.length - 1].invested, extInvested: r.sim.extInvested, cumDiv: +r.sim.cumDiv.toFixed(2), ret: r.sim.daily[r.sim.daily.length - 1].value / Math.max(1, r.sim.extInvested) - 1, /* 修复 B6：分母含月追加 */ divRatio: r.amount > 0 ? r.sim.cumDiv / r.amount * 100 : 0, yearlyDivs: yearlyDivsOf(r.sim.daily), monthlyFlow: r.sim.monthlyFlow, navSeries: navSeriesOf(r.sim.daily, r.amount), mSeries: mSeriesOf(r.sim.daily), yearly: simOne.yearlyOf(r.sim) }));
    const weightEvol = [];
