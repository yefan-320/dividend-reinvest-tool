...
(function () {
  ...
  async function renderDivCalendar() {
    ...
    el.innerHTML = `${holdInput}${targetInput}${targetHtml}${nearTxt}<div style="font-size:11px;color:var(--muted);margin-bottom:6px">未来 12 个月预计到账 <b>${cf.length} 个月</b>${yearTotal > 0 ? ' · 合计 <b style="color:var(--txt)">' + (yearTotal / 10000).toFixed(2) + ' 万</b>' : ''}（估=上年同期推算，未公告）</div>${rows.length ? rows.join('') : '<div class="hint">未来 12 个月无预计到账</div>'}`;
  ...
  async function openDiagnose(code, years) {
    ...
    try {
      ...
      $('#diagStats').innerHTML = `<div class="stats">
        ...
        <div class="stat"><div class="k">股息覆盖率(近2财年≈支付率)</div><div class="v">${cover != null ? cover.toFixed(2) : '—'}</div></div>
      ...
      if (!last) {
        ...
        zel.innerHTML = `<div class="zone-row">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">
            ...
            <span>当前 <b>${last.pct.toFixed(0)}%</b> 分位（${rangeWord}）</span>
          </div>
  ...
  function renderSellSignals(divs, kline) {
    ...
    const epsConsec = [];
    for (let i = 1; i < epsWindowed.length; i++) {
      if (epsWindowed[i].pct < 0 && epsWindowed[i - 1].pct < 0) epsConsec.push({ y: epsWindowed[i].y, p1: epsWindowed[i - 1].pct, p2: epsWindowed[i].pct });
    }
    ...
    const divConsec = [];
    for (let i = 1; i < yoyWindowed.length; i++) {
      // Q2（M29）：连续 2 年恶化=严格递减（pct<0 不含持平）
      if (yoyWindowed[i].pct != null && yoyWindowed[i].pct < 0 && yoyWindowed[i - 1].pct != null && yoyWindowed[i - 1].pct < 0) divConsec.push({ y: yoyWindowed[i].y, p1: yoyWindowed[i - 1].pct, p2: yoyWindowed[i].pct });
    }
    ...
    el.innerHTML = `<div style="margin-bottom:4px">${signals.map(s => `<div style="font-size:12px;margin:2px 0">${s.t}</div>`).join('')}</div>
      ...
      <div class="hint">口径：EPS/分红按报告期归组；连续 2 年（相邻年度）恶化才触发退出，单年波动仅关注；<b>信号仅看最近 5 个完整财年（滚动窗口，历史事件不永久触发）</b>；卖出信号是纪律参考，非自动执行</div>`;
  }
  ...
  function renderReportCard(code, divs, kline, extra) {
    ...
    const covTxt = cov != null ? cov.toFixed(2) + ' 倍' : '<span style="color:var(--muted)">数据不足</span>';
  ...
  function renderDecisionSummary(code, divs, kline) {
    ...
    body.innerHTML = `
      ...
      <div style="font-size:11px;color:var(--sub);margin-top:4px">股息率(TTM) ${yieldTxt} · 覆盖率 ${cov != null ? cov.toFixed(2) : '—'} · 分红CAGR ${cagr != null ? (cagr * 100).toFixed(1) + '%' : '—'}</div>
